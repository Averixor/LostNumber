...
## Warm charcoal / iron — deliberately desaturated (no violet cast on OLED).
const STONE_BLACK := Color("#100e0c")
const STONE_DEEP := Color("#1a1612")
const STONE_MID := Color("#2a241c")
const IRON := Color("#3c3630")
const BRONZE := Color("#8f6a36")
const BRONZE_DARK := Color("#4d351f")
const GOLD := Color("#d6ad58")
const GOLD_LIGHT := Color("#f3d58b")
## Kept for rare accent call-sites; chrome buttons must not fill/glow with this.
const CRYSTAL := Color("#8F55D6")
const CRYSTAL_LIGHT := Color("#C9A6F0")
const TEXT_IVORY := Color("#f7ead5")
const TEXT_MUTED := Color("#c4b8a8")
...
static func resolve_palette(theme_mgr: Node) -> Dictionary:
	...
	if theme_mgr != null and theme_mgr.has_method("get_palette"):
		if theme_mgr.has_method("uses_visual_skin"):
			raw = theme_mgr.call("get_palette", bool(theme_mgr.call("uses_visual_skin")))
		elif theme_mgr.has_method("get_visual_skin"):
			raw = theme_mgr.call("get_palette", theme_mgr.call("get_visual_skin") != null)
		else:
			raw = theme_mgr.call("get_palette", true)
	else:
		raw = {
			"bg": STONE_BLACK,
			"panel": STONE_DEEP,
			"primary": GOLD,
			"secondary": GOLD_LIGHT,
			"accent": GOLD_LIGHT,
			"danger": Color("#D0545C"),
	...
	return sanitize_chrome_palette(raw)


static func sanitize_chrome_palette(palette: Dictionary) -> Dictionary:
	## Force stone/gold chrome language even if a skin still declares violet primary.
	var out := palette.duplicate(true)
	out["primary"] = GOLD
	out["secondary"] = GOLD_LIGHT
	out["accent"] = GOLD_LIGHT
	var rim_raw = out.get("rim", GOLD)
	out["rim"] = GOLD if not (rim_raw is Color) else Color(rim_raw).lerp(GOLD, 0.35)
	# Hub/accent token historically named "crystal" — map to warm metal, not violet.
	out["crystal"] = GOLD.lerp(BRONZE, 0.25)
	out["glow"] = 0.45
	return out
...
static func tile_frame_tint(value: int, frozen: bool, rim: Color) -> Color:
	...
	if value >= 128:
		return Color(GOLD_LIGHT.lerp(BRONZE, 0.20), 0.98)
...
static func hud_panel(palette: Dictionary = {}) -> StyleBoxFlat:
	...
	style.shadow_color = Color(STONE_BLACK, 0.45)
	style.shadow_size = 8
...
static func card_panel(
	...
	style.shadow_color = Color(STONE_BLACK, 0.42 if complete else 0.28)
	style.shadow_size = 8 if complete else 5
	style.shadow_offset = Vector2(0, 2)
	return style


static func settings_row(palette: Dictionary = {}, compact: bool = false) -> StyleBoxFlat:
	var rim: Color = palette.get("rim", GOLD)
	var style := StyleBoxFlat.new()
	style.bg_color = Color(STONE_DEEP, 0.88)
	style.border_color = Color(rim, 0.62)
	style.set_border_width_all(1)
	style.set_corner_radius_all(8)
	var margin := 12 if compact else 14
	style.content_margin_left = margin
	style.content_margin_right = margin
	style.content_margin_top = 8 if compact else 10
	style.content_margin_bottom = 8 if compact else 10
	style.shadow_color = Color(STONE_BLACK, 0.35)
	style.shadow_size = 4
	style.shadow_offset = Vector2(0, 2)
	return style
...
static func _stone_button_base(palette: Dictionary, radius: int, vertical_margin: float) -> StyleBoxFlat:
	...
	style.shadow_color = Color(STONE_BLACK, 0.40)
	style.shadow_size = 4
	style.shadow_offset = Vector2(0, 2)
	return style
...
static func _apply_stone_button_state(style: StyleBoxFlat, palette: Dictionary, state: String) -> void:
	...
	match state:
		"hover":
			...
			style.border_color = Color(rim.lightened(0.12), 0.95)
...
static func booster_button(palette: Dictionary, active: bool, available: bool) -> StyleBoxFlat:
	## Active boosters: gold rim + warm stone fill — never violet bloom.
	var rim: Color = palette.get("rim", GOLD)
	var style := StyleBoxFlat.new()
	style.bg_color = Color(STONE_MID.lightened(0.06), 0.94) if active else Color(STONE_MID, 0.88)
	style.border_color = Color(GOLD_LIGHT, 0.95) if active else Color(rim, 0.72)
	...
	style.shadow_color = Color(STONE_BLACK, 0.40 if active else 0.28)
	style.shadow_size = 6 if active else 4
	style.shadow_offset = Vector2(0, 2)
	return style
