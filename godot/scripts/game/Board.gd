...
func _build_grid() -> void:
	...
	for child in get_children():
		child.queue_free()
	custom_minimum_size = Vector2(GRID_W * cell_size.x + (GRID_W - 1) * cell_gap, GRID_H * cell_size.y + (GRID_H - 1) * cell_gap)
	for x in GRID_W:
		var col: Array = []
		for y in GRID_H:
...
func _collect_cells_along_pointer_path(local_pos: Vector2) -> Array[Vector2i]:
	...
	if not _last_pointer_local.is_finite():
		_last_pointer_local = local_pos
		return [cell]
	var delta := local_pos - _last_pointer_local
	var distance := delta.length()
	var step := maxf(minf(cell_size.x, cell_size.y) * 0.35, 10.0)
	if distance < step * 0.45:
		_last_pointer_local = local_pos
		return [cell]
	var count := maxi(1, int(ceil(distance / step)))
	for i in range(1, count + 1):
		var sample := _last_pointer_local.lerp(local_pos, float(i) / float(count))
		var sample_cell := _cell_at_local(sample)
		if sample_cell.x >= 0 and (cells.is_empty() or cells.back() != sample_cell):
...
func _cell_at_local(local_pos: Vector2) -> Vector2i:
	...
	var inside := in_cell.x >= -gap_tolerance and in_cell.y >= -gap_tolerance and in_cell.x <= cell_size.x + gap_tolerance and in_cell.y <= cell_size.y + gap_tolerance
	return Vector2i(x, y) if inside else Vector2i(-1, -1)
...
func _update_chain_visual() -> void:
	if state == null or _chain_layer == null:
		return
	...
	_chain_layer.set_chain_points(pts, state.can_finish_current_chain(), str(_chain_total()), last)
