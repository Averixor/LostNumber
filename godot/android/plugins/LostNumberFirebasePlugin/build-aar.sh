#!/usr/bin/env bash
# Build LostNumberFirebasePlugin-release.aar for Godot export (.gdap local binary).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")" && pwd)"
GODOT_BUILD="$ROOT/../../build"
PLUGIN_DIR="$ROOT"
AAR_OUT="$PLUGIN_DIR/LostNumberFirebasePlugin-release.aar"
...
if [[ ! -f "$GODOT_BUILD/libs/release/godot-lib.template_release.aar" ]]; then
  ...
  echo "  npm run godot:android:debug" >&2
  exit 1
fi
