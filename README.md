...
Після **40** попередньо згенерованих рівнів гра продовжує процедурно: ціль рівня визначається через **`getLevelConfig(levelIndex)`** (0-based індекс). У збереженні лишається **`currentLevel`**; ціль відновлюється детерміновано після reload. Ранні рівні (1–40) не змінюють баланс.
