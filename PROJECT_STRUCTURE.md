...
**Primary ship target:** Godot 4.5 Android (`godot/`, `npm run godot:android:release`). Web/Capacitor — візуальний еталон і legacy. Канон: `docs/en/SOURCE_OF_TRUTH.md`.
...
Збірка web-артефакту (legacy/reference): **`npm run build:pages`** → `_site/`. Для розробки web UI — `index.html` або `npx serve .`.
...
| `scenes/MainMenu.tscn`, `Game.tscn`, `Settings.tscn`, `SkinPreview.tscn`, … | Екрани; монтуються в ScreenRoot через ScreenRouter (`skin_preview` у `ScreenRouter.SCREENS`)                                                                                       |
| `scenes/components/`                                                        | BackgroundLayer, NeonButton, ScreenTransition                                                                                                                                      |
| `scripts/ui/ScreenRouter.gd`                                                | Autoload: push/replace/go_back, back-stack, fade; screens incl. `skin_preview`                                                                                                     |
| `scripts/core/LevelManager.gd`                                              | 40 preset levels + procedural endless (`MANUAL_LEVEL_COUNT := 40`)                                                                                                                 |
| `scripts/ui/SkinPreview.gd`, `ImagePickerHelper.gd`                         | Custom background preview + native gallery/file picker (Android/desktop)                                                                                                           |
...
| `assets/ui/backgrounds/{dark,light,twilight}/`                              | 6+6+6 фонів меню (twilight у коді; UI toggle — dawn/dusk only)                                                                                                                     |
